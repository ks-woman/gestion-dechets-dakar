        ...
        <div class="p-6">
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl font-bold">
                                ...
                                @elseif($abonnement && $abonnement->estEnEssai())
                        ...
                        <div class="text-right">
                            ...
                            <p class="font-semibold">
                                @if ($abonnement && $abonnement->date_debut_abonnement)
                                    {{ $abonnement->date_debut_abonnement->addMonth()->format('d/m/Y') }}
                                @else
                    ...
                    @if ($abonnement && $abonnement->estEnEssai())
                        <div class="mt-3 bg-blue-50 border border-blue-200 rounded-lg p-3">
                            <p class="text-sm text-blue-700">
                                Il vous reste <strong>{{ now()->diffInDays($abonnement->date_fin_essai) }}</strong> jours
                                d'essai gratuit.
            ...
            <div class="grid md:grid-cols-2 gap-6 mb-8">
                <div class="border-2 border-emerald-500 rounded-2xl p-6 bg-emerald-50">
                    ...
                    @if (!($abonnement && $abonnement->estActif()))
                        <form method="POST" action="{{ route('abonnement.souscrire') }}" class="mt-4">
                            @csrf
                            <button type="submit"
            ...
            <div class="border-t pt-6">
                ...
                @if ($paiements->count() > 0)
                    <div class="space-y-2">
                        @foreach ($paiements as $paiement)
                            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                <div>
                                    <p class="font-medium">{{ $paiement->date_paiement->format('d/m/Y') }}</p>
                                    <p class="text-xs text-gray-500">{{ $paiement->mode_paiement }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="font-semibold text-emerald-600">{{ number_format($paiement->montant, 0) }}
                                        FCFA</p>
                    ...
                    <div class="mt-3">
                        <a href="{{ route('abonnement.historique') }}" class="text-emerald-600 hover:underline text-sm">
