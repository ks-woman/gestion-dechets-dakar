...
@section('content')
            ...
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl shadow p-4 text-white">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-2xl font-bold">{{ $totalCollectes }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-xl shadow p-4 text-white">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-2xl font-bold">{{ number_format($totalRecyclable, 1) }}</p>
                    </div>
                    <div class="text-2xl"></div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-xl shadow p-4 text-white">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-2xl font-bold">{{ number_format($totalOrganique, 1) }}</p>
                    </div>
                    <div class="text-2xl"></div>
                </div>
            </div>
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        @foreach ($collectes as $collecte)
                            <tr class="border-b">
                                <td class="py-2">{{ \Carbon\Carbon::parse($collecte->date_collecte)->format('d/m/Y') }}
                                </td>
                                <td class="py-2">{{ number_format($collecte->poids_recyclable, 1) }} kg</td>
                                <td class="py-2">{{ number_format($collecte->poids_organique, 1) }} kg</td>
                                <td class="py-2">{{ number_format($collecte->poids_residuel, 1) }} kg</td>
                                <td class="py-2">+{{ $collecte->points_obtenus }} pts</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <script>
        ...
        new Chart(collectesCtx, {
            type: 'line',
            data: {
                labels: {!! json_encode(array_keys($collectesParMois)) !!},
                datasets: [{
                    label: 'Nombre de collectes',
                    data: {!! json_encode(array_values($collectesParMois)) !!},
                    borderColor: '#3b82f6',
        ...
        new Chart(pointsCtx, {
            type: 'bar',
            data: {
                labels: {!! json_encode(array_keys($pointsParMois)) !!},
                datasets: [{
                    label: 'Points gagnés',
                    data: {!! json_encode(array_values($pointsParMois)) !!},
